...
export function GameField({ gameContext }) {
  // Extract necessary functions and state from gameContext
  const {
    game,
    players,
    handlePlayerClick,
  ...
  // Remova o código de fetch e use as funções do contexto
  ...
  const handleSelectPosition = (zone, positionIndex, player) => {
    ...
    if (player) {
      ...
      // Call the updatePlayerPosition function from gameContext
      // Esta função já cuidará de atualizar o estado local e enviar para o servidor
  ...
  const handleAssignPlayer = (player) => {
    if (dialogPosition) {
      ...
      try {
        // Call the context function to update position locally and on the server
          ...
          {selectedPlayer && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold ${selectedPlayer.gender === "male" ? "bg-blue-400" : "bg-pink-400"}`}
                >
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-yellow-600">
                    Clique em uma posição no campo para colocar este jogador
                  </p>
                </div>
              </div>
            ...
            <div className="absolute top-0 left-0 flex h-full w-1/2 flex-col">
              ...
              <div className="relative flex-1 p-4">
                {attackPositions.map((position, index) => {
                  // Find the player at this specific position
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4 max-h-[300px] overflow-y-auto">
              {substitutes.map((player) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold ${player.gender === "male" ? "bg-blue-400" : "bg-pink-400"}`}
                  >
                    {player.name.charAt(0)}
                  </div>
